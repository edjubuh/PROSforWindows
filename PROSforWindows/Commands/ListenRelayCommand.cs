...
{
    public class ListenRelayCommand : ICommand
    {
        ...
        public ListenRelayCommand()
        {
            ControlEvent = new List<WeakReference>();
        }

        public ListenRelayCommand(Dispatcher dispatch) : this()
        {
            Dispatcher = dispatch;
        }
        ...
        public ListenRelayCommand ListenOn<TObservedType, TPropertyType>(TObservedType viewModel, Expression<Func<TObservedType, TPropertyType>> propertyExpression) where TObservedType : INotifyPropertyChanged
        {
